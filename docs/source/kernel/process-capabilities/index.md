...
现在 Cloud Native 的容器，大都是在 Linux 上运行。而 Linux 的权限（Capabilities）说到底，是以进程（或更准确说是线程）为授权单位来控制的。而博学如你也知道，容器在 Linux 角度来说，不过是放在子 kernel namespace 的进程集。所以只要了解 Linux 权限（Capabilities）机制，就了解容器权限的核心。
