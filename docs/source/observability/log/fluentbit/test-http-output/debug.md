...
docker stop httpbin
docker run -it --rm -d -p 8080:80 --network=httpbin --name=httpbin --hostname=httpbin kennethreitz/httpbin
curl -X POST "http://localhost:8080/post" -H "accept: application/json"

docker stop nginx
docker rm nginx
docker run -d --rm -p 443:443 --network=httpbin --name=nginx --hostname=nginx \
...
docker exec nginx curl -v -k -X POST "https://localhost:443/post" -H "accept: application/json"
...
curl -v -k -X POST "https://localhost/post" -H "accept: application/json"
