...
![img](linux-net-stack-implementation-rx-zh.assets/rx-overview-16680445350494.png)
...
![img](linux-net-stack-implementation-rx-zh.assets/run_ksoftirqd-166804471904334.png)
...
![img](linux-net-stack-implementation-rx-zh.assets/uu_do_softirq-166804472731537.png)
...
如果是 GRO\_NORMAL，会调用 `gro_normal_one()`，它会更新当前 napi->rx\_count 计数， 当数量足够多时，将调用 `gro_normal_list()`，后者会将包一次性都送到协议栈：
...
![img](linux-net-stack-implementation-rx-zh.assets/napi_gro_receive-166804482803852.png)
...
![img](linux-net-stack-implementation-rx-zh.assets/rx-overview-166804483979155.png)

Fig. Steps of Linux kernel receiving data process and the corresponding chapters in this post

## 7.1 流程图

![img](linux-net-stack-implementation-rx-zh.assets/netif_receive_skb_list_internal-166804484786558.png)
...
2. 启用了 RPS：先 enqueue\_to\_backlog，后面的逻辑和 1 一样。
...
![img](linux-net-stack-implementation-rx-zh.assets/netif_receive_skb_list_internal-166804486818261.png)
...
更多关于 XDP 的内容： [XDP (eXpress Data Path)：在操作系统内核中实现快速、可编程包处理（ACM，2018）](http://arthurchiao.art/blog/xdp-paper-acm-2018-zh/)。
...
如果对 packet 如何经过 pcap 有兴趣，可以阅读 [net/packet/af\_packet.c](https://github.com/torvalds/linux/blob/v5.10/net/packet/af_packet.c)。
...
1. [（译）《Linux 高级路由与流量控制手册（2012）》第九章：用 tc qdisc 管理 Linux 网络带宽](http://arthurchiao.art/blog/lartc-qdisc-zh/)
2. [（译）基于 BPF/XDP 实现 K8s Service 负载均衡 (LPC, 2020)](http://arthurchiao.art/blog/cilium-k8s-service-lb-zh/)
3. [（译）利用 eBPF 支撑大规模 K8s Service (LPC, 2019)](http://arthurchiao.art/blog/cilium-scale-k8s-service-with-bpf-zh/)
...
- 更多内容见 [Differentiate three types of eBPF redirections](http://arthurchiao.art/blog/differentiate-bpf-redirects/)。
...
1. [(译) 深入理解 iptables 和 netfilter 架构](http://arthurchiao.art/blog/deep-dive-into-iptables-and-netfilter-arch-zh/)
2. [连接跟踪（conntrack）：原理、应用及 Linux 内核实现](http://arthurchiao.art/blog/conntrack-design-and-implementation-zh/)

### 7.3.8 进入 L3 协议层

通过 **`skb->dev->rx_handler(&skb)`** 进入 L3 ingress 处理，例如 IPv4 处理逻辑。

### 7.3.9 更新 softnet\_data 统计
...
![img](linux-net-stack-implementation-rx-zh.assets/rx-overview-166804489289464.png)
...
![img](linux-net-stack-implementation-rx-zh.assets/inet_init-166804490010067.png)
...
![img](linux-net-stack-implementation-rx-zh.assets/l3-processing-stack-166804491194670.png)
...
![img](linux-net-stack-implementation-rx-zh.assets/rx-overview-166804492605373.png)
...
![img](linux-net-stack-implementation-rx-zh.assets/uu_udp4_lib_rcv-166804493465576.png)
...
1. [Linux 网络栈监控和调优：接收数据（2016）](http://arthurchiao.art/blog/linux-net-stack-implementation-rx-zh/%7Blink%20_posts/2018-12-05-tuning-stack-rx-zh.md%7D)
2. [Monitoring Linux Network Stack](http://arthurchiao.art/blog/monitoring-network-stack/)
3. [(译) 深入理解 Cilium 的 eBPF 收发包路径（datapath）](http://arthurchiao.art/blog/understanding-ebpf-datapath-in-cilium-zh/)
4. [XDP (eXpress Data Path)：在操作系统内核中实现快速、可编程包处理（ACM，2018）](http://arthurchiao.art/blog/xdp-paper-acm-2018-zh/)
